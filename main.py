...
def optimize(selected_pipeline):
    ...
    dict_assignment_resource_util, overall_future_utilization = future_utilization_metrics(RESOURCES_future)
    resources = list(dict_assignment_resource_util.keys())
    ut_factor = list(dict_assignment_resource_util.values())
    dict_assignment_resource_util_new = {"Resources": resources, "Utilization %": ut_factor}
    
    optimization_results = pd.DataFrame.from_dict(dict_assignment_resource_util_new)
    opt_table = [optimization_results.to_html(classes='cols_rows_df')]
    opt_title = optimization_results.columns.values
    ...
    return render_template('dashboard.html', graphJSON=graphJSON, hire_res_info = hire_res_info,
                                             tables=opt_table, titles = opt_title, 
                                             overall_fu = f"{round(overall_future_utilization,2)} %")
